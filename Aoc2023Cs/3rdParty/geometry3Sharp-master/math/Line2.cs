...
namespace g3
{
    public struct Line2d : IEquatable<Line2d>
    {
        ...
        /// </summary>
        /// <returns></returns>
        public double IntersectionPointProjected(ref Line2d other, double dotThresh = MathUtil.ZeroTolerance)
        {
        ...
        {
            double s = IntersectionPointProjected(ref other, dotThresh);
            if (s==Double.MaxValue) return Vector2d.MaxValue;
            return Origin + s * Direction;
        }

        public Vector2d IntersectionPoint(ref Line2d other, out double s, double dotThresh = MathUtil.ZeroTolerance)
        {
            s = IntersectionPointProjected(ref other, dotThresh);
            if (s==Double.MaxValue) return Vector2d.MaxValue;
            return Origin + s * Direction;
        }
