...

{
    ...
    public struct Pipe
    {
        ...
        public Pipe(Vec2 position, char type)
        {
            ...
            switch (type)
            {
                case '|' : Connect(Connection.North, Connection.South);
                    break;
                case '-' : Connect(Connection.East, Connection.West);
                    break;
                case 'L' : Connect(Connection.North, Connection.East);
                    break;
                case 'J' : Connect(Connection.North, Connection.West);
                    break;
                case '7' : Connect(Connection.West, Connection.South);
                    break;
                case 'F' : Connect(Connection.East, Connection.South);
                    break;
                case '.' : Connect(Connection.None, Connection.None);
                    break;
            }
        }
